...
class Book:
    def __init__(self, name, author, year_of_publication):
        ...
        self.age = current_year - self.year_of_publication
        
        if self.age >= 400:
            self.status = "stored on floor 2 "
        elif self.age >= 200:
            self.status = "stored on floor 1 "
        elif self.age >= 100:
            self.status = "stored on floor 0 "
        else:
            self.status = "not considered antique enough for this library"
